...
total_frames = 48391;
model_definition = 'D:\face project\pain\center_loss_models\face_deploy_all_verification.prototxt';
weights = 'D:\face project\pain\face_model_my.caffemodel';
net = caffe.Net(model_definition, weights, 'test');

batch_size = 100;
total_iter = ceil(total_frames/batch_size);

features = zeros(512, total_iter*batch_size);
labels = zeros(2, total_iter*batch_size);

for i=1:total_iter
    disp([i total_iter]);
    f = net.forward({});
    features(:,(i-1)*batch_size + 1: i*batch_size) = f{1};
